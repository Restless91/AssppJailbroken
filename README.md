...
版本 `0.1.12` 为设备串行任务增加加密恢复存储。排队顺序和必要的 CDN/SINF 恢复材料分别持久化，
敏感材料使用设备本地 AES-GCM 密钥加密并以 `0600` 权限保存；daemon 重启后任务按原顺序重新排队。
前端显示队列位次，可恢复的失败任务支持“重新排队”，完成或删除任务后自动销毁恢复材料。

版本 `0.1.13` 增加稳定 `.part` 文件和 HTTP Range 断点续传。暂停、网络中断或 daemon 重启后会携带
`Range`/`If-Range` 从已保存字节继续下载；严格校验 `206 Content-Range`，服务器忽略范围或内容已变化时
安全重置为完整下载。部分文件和校验元数据使用 `0600` 权限，完成后原子替换并自动清理。
...
| Dopamine/rootless | 0.1.10 | `wiki.qaq.unfaird_0.1.10_iphoneos-arm64.deb` | `1877ac7e14e5cb1ea871d9a893707a6fce6ee601449605f9a98d10f27c9d7b43` |
