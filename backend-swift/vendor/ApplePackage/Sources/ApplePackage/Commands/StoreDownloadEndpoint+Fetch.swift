...
extension StoreDownloadEndpoint {
    ...
    static func fetchProductWithFallback(
        ...
        var dict: [String: Any]
        do {
            dict = try await StoreDownloadEndpoint.volumeStore.fetchProduct(
                client: client,
                account: &account,
                app: app,
            ...
            // The volumeStore host is intermittently fronted by an Apple/CDN
            // gateway that returns 502/204 or HTML instead of a plist. Those
            // responses are transport failures, not account failures. The
            // redownload dispatch endpoint uses the same authenticated
            // material and is a reliable fallback for this condition.
            guard isTransientGatewayError(error) else { throw error }
            APLogger.debug("store: volumeStore gateway failure, retrying via redownload endpoint")
            return try await StoreDownloadEndpoint.redownload.fetchProduct(
                client: client,
                account: &account,
                app: app,
                deviceIdentifier: deviceIdentifier,
    ...
    private static func isTransientGatewayError(_ error: Error) -> Bool {
        ...
        return message.contains("status 502") ||
