...
public enum Authenticator {
    private static let nativeAuthEndpoint = URL(string: "https://auth.itunes.apple.com/auth/v1/native/fast/")!
    ...
    public static func authenticate(
        ...
        while currentAttempt <= 2, redirectAttempt <= 3 {
            ...
            do {
                ...
                // Apple may advertise the legacy buy.itunes.apple.com
                // endpoint during a rollout, but return a gateway response
                // without a Location header. Retry once through the native
                // authentication endpoint before parsing the body as a plist.
                let legacyEndpoint = requestEndpoint.host == "buy.itunes.apple.com"
                let gatewayResponse = response.status.code >= 500 || response.status == .noContent
                let redirectWithoutLocation = response.status == .movedPermanently ||
                    response.status == .found ||
                    response.status == .seeOther ||
                    response.status == .temporaryRedirect ||
                    response.status == .permanentRedirect
                if legacyEndpoint && (gatewayResponse || (redirectWithoutLocation && response.headers.first(name: "location") == nil)) {
                    requestEndpoint = Self.nativeAuthEndpoint
                    redirectAttempt += 1
                    continue
                }
